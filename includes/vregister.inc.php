<?php
	...
	if(isset($_POST['username'], $_POST['email'], $_POST['p'], $_POST['anrede'], $_POST['vname'], $_POST['nname'], $_POST['str'], $_POST['nr'], $_POST['plz'], $_POST['ort'], $_POST['agb'], $_POST['abo'])) {
		...
		$prep_stmt = "SELECT id FROM _main_vmembers WHERE email = ? LIMIT 1";
		...
		if(empty($error_msg)) {
			...
			if($insert_stmt = $mysqli->prepare("INSERT INTO _main_vmembers (username, email, password, salt, anrede, vname, nname, str, nr, plz, ort, agb, abo, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
				$insert_stmt->bind_param('ssssssssssssss', $username, $email, $password, $random_salt, $anrede, $vname, $nname, $str, $nr, $plz, $ort, $agb, $abo, $color);
