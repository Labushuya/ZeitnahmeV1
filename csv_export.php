<? error_reporting(E_ALL);
	...
	if(isset($_GET['rid']) && !empty($_GET['rid'])) {
		...
		// Erstelle Arrays
		// Vorbereitungs-Container /!\
		/*
		Header 		--> array['#',		'Fahrtzeit',	'Abweichung']
		1. Schritt		--> array1[$i]	
							array2[$i]		
							array3[$i]
		2. Schritt		--> array['array1[$i]',	'array2[$i]',	'array3[$i]',]
		Bereinigt	--> array['1',		'00:03:20,50',	'00:00:03,06']
		*/
		$csv_sanitized_info = array();
		
		// CSV Export Tabellenkopf
		$csv_header = array();
				...
				if($rd_zentry == 0) {
					...
					if($rd_tpos == 2) {
						...
						*/
						...
						// Erstelle Tabellenkopf
						$csv_header[] = "Ziel"; 
						$csv_header[] = "Fahrtzeit"; 
						$csv_header[] = "Abweichung";
						$csv_header[] = "Sollzeit";
						
						// PUT TABLE CONTENT IN CSV
						...
						for($a = 1; $a < (count($con_tmember) + 1); $a++) {
							...
							$csv_sanitized_info[$con_tmember[$a]][0] = $con_tmember[$a];
							...
							if($numrow_pre == 1) {
								...
								$csv_sanitized_info[$con_tmember[$a]][1] = "'" . $getrow_pre['t_realtime'];
							...
							// Es wurde keine Startzeit gefunden
								...
								// Erstelle Tablleninhalt
								$csv_sanitized_info[$con_tmember[$a]][1] = "'---";
							...
							if($numrow_seq == 1) {
								// Erstelle Tablleninhalt
								$csv_sanitized_info[$con_tmember[$a]][2] = "'" . $getrow_seq['t_realtime'];
							...
							// Es wurde keine Zielzeit gefunden
								...
								// Erstelle Tablleninhalt
								$csv_sanitized_info[$con_tmember[$a]][2] = "'---";
							...
							// Erstelle Tabelleninhalt
							$csv_sanitized_info[$con_tmember[$a]][3] = "'" . $array_fahrtzeit_konvert[$a];
							$csv_sanitized_info[$con_tmember[$a]][4] = "'" . $array_abweichung_konvert[$a];
							$csv_sanitized_info[$con_tmember[$a]][5] = "'" . $sollzeit;						
							
							// Packe Tabelleninhalt in CSV
							fputcsv($output, $csv_sanitized_info[$con_tmember[$a]], ';');
						}
					// Prüfung mit variabler Anzahl an Zwischenzeiten
						...
						for($a = 0; $a < ($rd_tpos - 1); $a++) {
							$csv_header[] = "Fahrtzeit " . ($a + 1);
						}
						
						// Erstelle CSV Header
						$csv_header[] = "Fahrtzeit gesamt";
							
						for($a = 0; $a < ($rd_tpos - 1); $a++) {
							// Erstelle CSV Header
							$csv_header[] = "Abweichung " . ($a + 1);
						}
						
						// Erstelle CSV Header
						$csv_header[] = "Abweichung gesamt";
							
						for($a = 0; $a < ($rd_tpos - 1); $a++) {
							// Erstelle CSV Header
							$csv_header[] = "Sollzeit " . ($a + 1);
						}
						...
						for($b = 1; $b < (count($con_tmember) + 1); $b++) {
							// BUILD TABLE CONTENT
							$csv_sanitized_info[$con_tmember[$b]][0] = $con_tmember[$b];
							...
							if($numrow_pre == 1) {
								// Erstelle Tabelleninhalt
								$csv_sanitized_info[$con_tmember[$b]][1] = "'" . $getrow_pre['t_realtime'];
							...
							// Startzeit wurde nicht gefunden
								...
								// Erstelle Tabelleninhalt
								$csv_sanitized_info[$con_tmember[$b]][1] = "'---";
							...
							$next_index = count($csv_sanitized_info[$con_tmember[$b]]);
							...
							for($c = 0; $c < ($rd_tpos - 2); $c++) {
								...
								if($numrow_opt == 1) {
									// Erstelle Tablleninhalt
									$csv_sanitized_info[$con_tmember[$b]][(2 + $c)] = "'" . $getrow_opt['t_realtime'];
								...
								// Zwischenzeit X wurde nicht gefunden
									...
									// Erstelle Tablleninhalt
									$csv_sanitized_info[$con_tmember[$b]][(2 + $c)] = "'---";
							...
							if($numrow_seq == 1) {
								...
								// BUILD TABLE CONTENT
								$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $getrow_seq['t_realtime'];
							...
							// Zielzeit wurde nicht gefunden
								...
								// BUILD TABLE CONTENT
								$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'---";
							...
							for($d = 0; $d < ($rd_tpos - 1); $d++) {
								...
								*/								
								...
								if($rd_calc == 1) {
									if($d >= 0 AND $d < (($rd_tpos - 1) - 1)) {
										// Erstelle Tabelleninhalt				
										$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_fahrtzeit_konvert[$con_tmember[$b]]["fz" . $d];
									} elseif($d >= 0 AND $d == (($rd_tpos - 1) - 1)) {
										// Erstelle Tabelleninhalt			
										$csv_sanitized_info[$con_tmember[$b]][$next_index] = '┗━►';
									}										
							...
							if($rd_calc == 1) {
								// Erstelle Tabelleninhalt			
								$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_fahrtzeit_konvert[$con_tmember[$b]]["fz" . (($rd_tpos - 1) - 1)];
							} elseif($rd_calc == 2) {
								// Erstelle Tabelleninhalt			
								$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_fahrtzeit_konvert_gesamt[$con_tmember[$b]];
							}
							...
							for($e = 0; $e < ($rd_tpos - 1); $e++) {
								if($e < ($rd_tpos - 1)) {
									// Erstelle Tabelleninhalt			
									$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_abweichung_konvert[$con_tmember[$b]]["ab" . $e];
								}
							...
							// Erstelle Tabelleninhalt			
							$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_abweichung_konvert_gesamt[$con_tmember[$b]];
							...
							for($f = 0; $f < ($rd_tpos - 1); $f++) {
								if($f < ($rd_tpos - 1)) {
									// Erstelle Tabelleninhalt			
									$csv_sanitized_info[$con_tmember[$b]][$next_index] = "'" . $array_sollzeit_konvert[$f];
								}
							...
							fputcsv($output, $csv_sanitized_info[$con_tmember[$b]], ';');
						}
					}					
				// Eine Prüfung vom Typ "Sprint"	
					...
					*/
					...
					for($a = 1; $a < (count($con_tmember) + 1); $a++) {
						...
						// Erstelle Tabelleninhalt
						$csv_sanitized_info[$con_tmember[$a]][0] = "'" . $con_tmember[$a];
						$csv_sanitized_info[$con_tmember[$a]][1] = "'" . $array_fahrtzeit_konvert[$a];
						$csv_sanitized_info[$con_tmember[$a]][2] = "'" . $array_abweichung_konvert[$a];
						$csv_sanitized_info[$con_tmember[$a]][3] = "'" . $sollzeit;
